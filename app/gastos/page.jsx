    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .btn { background: #1e40af; color: #fff; padding: 10px 16px; }
        .back { background: #fff; color: #2563eb; border: 1px solid rgba(37,99,235,.18); padding: 10px 14px; }
        ...
        .filters .input, .filters .select, .modal-input { padding: 10px 12px; border: 1px solid rgba(37,99,235,.18); background: #fff; }
        ...
        .badge { display: inline-block; padding: 4px 8px; border-radius: 999px; background: rgba(37,99,235,.08); color: #2563eb; font-size: 12px; }
        ...
        .modal-overlay { position: fixed; inset: 0; background: rgba(15,31,61,.45); display: grid; place-items: center; padding: 24px; }
        ...
        .check { display: flex; align-items: center; gap: 8px; color: #0f1f3d; }
      ...
      <div className="page">
        <div className="wrap">
          ...
          <div className="card filters">
            ...
            <input className="input" type="number" min="1" max="12" value={filters.mes} onChange={(e) => updateFilter("mes", e.target.value)} />
            <input className="input" type="number" min="2000" max="9999" value={filters.year} onChange={(e) => updateFilter("year", e.target.value)} />
            <button className="btn" onClick={() => loadGastos(filters)}>Filtrar</button>
          </div>

          <div className="summary">
            <div className="card accent">
              <div className="label">Total gastos del periodo</div>
          ...
          <div className="card table-card">
            {isLoading ? (
              ...
              <div className="empty">No hay gastos para el periodo seleccionado.</div>
            ) : (
              <table>
                <thead>
                  <tr>
                ...
                <tbody>
                  {gastos.map((gasto) => (
                    <tr key={gasto.id}>
                      ...
                      <td>{String(gasto.mes).padStart(2, "0")}/{gasto.year}</td>
                      <td className="money">${Number(gasto.monto).toLocaleString("es-AR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                      <td>{gasto.se_repite_mensualmente ? <span className="badge">Sí</span> : "No"}</td>
                      <td>
                        <div className="actions">
                          <Link href={`/gastos/${gasto.id}`} className="back ghost">ver más</Link>
                          <button className="back ghost" onClick={() => openEdit(gasto)}>editar</button>
                          <button className="back danger" onClick={() => deleteGasto(gasto.id)}>eliminar</button>
                        </div>
                      </td>
                    </tr>
                  ))}
          ...
          <form onSubmit={handleSubmit}>
            <div className="modal-body">
              ...
              <label className="check">
                <input type="checkbox" checked={form.se_repite_mensualmente} onChange={(e) => updateField("se_repite_mensualmente", e.target.checked)} />
